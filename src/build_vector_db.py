...
from .config import build_embeddings, CHROMA_COLLECTION, CHROMA_DIR, DATASET_DIR
...
def build_and_persist_chroma(docs, persist_directory=CHROMA_DIR, batch_size=64):
    ...
    # Chunk docs for progress reporting. Keep batches modest: the Ollama embed
    # client can race its local connection port on very large single requests
    # (e.g. 512 rows), surfacing as dial tcp 127.0.0.1:<ephemeral>: refused.
